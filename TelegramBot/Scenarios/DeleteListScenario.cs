        ...
        async Task<ScenarioResult> IScenario.HandleMessageAsync(ITelegramBotClient bot, ScenarioContext context, Update update, CancellationToken ct)
        {
            switch (context.CurrentStep)
            {
                case null:
                    ...
                    if (toDoLists.Any())
                    {
                        context.Data["lastMessageId"] = (await bot.SendMessage(update.CallbackQuery.Message.Chat.Id, "Выберете список для удаления:", Telegram.Bot.Types.Enums.ParseMode.None, replyMarkup: Keyboards.ToDoListKeyboard(toDoLists, "deletelistbyid"), cancellationToken: ct)).Id;
                        context.CurrentStep = "Approve";
                        return ScenarioResult.Transition;
                    }
                    else return ScenarioResult.Completed;
                case "Approve":
                    await bot.SendMessage(update.CallbackQuery.Message.Chat.Id, $"Подтверждаете удаление списка {((ToDoList)context.Data["ToDoList"]).Name} и всех его задач?", Telegram.Bot.Types.Enums.ParseMode.None, replyMarkup: Keyboards.YesNoKeyboard("deletelistbyid", null), cancellationToken: ct);
                    context.CurrentStep = "Delete";
                    return ScenarioResult.Transition;
                default:
                    if (context.Data["Approve"].ToString() == "deletelistbyidyes")
                    {
                        ...
                        await bot.EditMessageReplyMarkup(update.CallbackQuery.Message.Chat.Id, (int)context.Data["MessageId"], Keyboards.ShowToDoListKeyboard(await ToDoListService.GetUserListsAsync(((ToDoUser)context.Data["ToDoUser"]).UserId, ct)), cancellationToken: ct);
